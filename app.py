...
def main():
    ...
    carmodel_sl = center.selectbox('Car Model', carmodel_lbl, index=carmodel_lbl.index('diesel'))
    symboling = right.number_input('Symboling', step=1.0, format='%.1f', value=-1.0)
    fueltype_sl = left.selectbox('Fuel Type', fueltype_lbl, index=fueltype_lbl.index('gas'))
    aspiration_sl = center.selectbox('Aspiration', aspiration_lbl, index=aspiration_lbl.index('turbo'))
    ...
    drivewheel_sl = center.selectbox('Drive Wheel :white_check_mark:', drivewheel_lbl, index=drivewheel_lbl.index('rwd'))
    enginelocation_sl = right.selectbox('Engine Location', enginelocation_lbl, index=enginelocation_lbl.index('front'))
    wheelbase = left.number_input('Wheel Base :white_check_mark:',  step=1.0, format='%.1f', value=104.3)
    carlength = center.number_input('Car Length :white_check_mark:',  step=1.0, format='%.1f', value=188.8)
    ...
    curbweight  = center.number_input('Curb Weight :white_check_mark:',  step=1.0, format='%.1f', value=3157.0)
    enginetype_sl = right.selectbox('Engine Type', enginetype_lbl, index=enginetype_lbl.index('ohc'))
    cylindernumber_sl = left.selectbox('Cylinder Number', cylindernumber_lbl, index=cylindernumber_lbl.index('four'))
    enginesize = center.number_input('Engine Size :white_check_mark:',  step=1.0, format='%.1f', value=130.0)
    ...
    stroke = center.number_input('Stroke',  step=1.0, format='%.2f', value=3.15)
    compressionratio = right.number_input('compressionratio',  step=1.0, format='%.1f', value=7.5)
    horsepower = left.number_input('Horse Power :white_check_mark:',  step=1.0, format='%.1f', value=162.0)
    peakrpm = center.number_input('peakrpm',  step=1.0, format='%.1f', value=5100.0)
